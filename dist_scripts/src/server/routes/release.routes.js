...
router.post('/', async (req, res) => {
    try {
        const { title, artworkUrl, upc, originalReleaseDate, goLiveDate, albumCategoryId, contentTypeId, cLine, subLabelId, publisherId, primaryArtistId } = req.body;
        const userId = req.user.id;
        const release = await db_1.db.release.create({
            data: {
                title,
                artworkUrl,
                upc,
...
router.get('/', async (req, res) => {
    try {
        ...
        const releases = await db_1.db.release.findMany({
        ...
        const releasesWithUrls = await Promise.all(releases.map(async (release) => {
            let artworkSignedUrl = null;
            if (release.artworkUrl) {
                try {
                    artworkSignedUrl = await (0, storage_1.generateDownloadUrl)(release.artworkUrl, 3600);
                }
                catch (error) {
...
router.put('/:id', async (req, res) => {
    try {
        ...
        const userId = req.user.id;
        const isAdmin = req.user.role === 'ADMIN';
        const release = await db_1.db.release.findUnique({ where: { id } });
        if (!release) {
            return res.status(404).json({ error: 'Release not found' });
        }
        if (!isAdmin && release.userId !== userId) {
        ...
        const updatedRelease = await db_1.db.release.update({
            where: { id },
...
router.post('/:id/tracks', async (req, res) => {
    try {
        ...
        const track = await db_1.db.track.create({
            data: {
                title,
                duration,
                genre,
