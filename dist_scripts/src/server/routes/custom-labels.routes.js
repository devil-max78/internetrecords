...
router.get('/', async (req, res) => {
    try {
        ...
        const requests = await db_1.db.customLabelRequest.findMany({ where });
        res.json(requests);
    }
    catch (error) {
...
router.post('/', async (req, res) => {
    try {
        ...
        const existingLabels = await db_1.db.subLabel.findMany({
            where: { name }
        });
        if (existingLabels.length > 0) {
            return res.status(409).json({ error: 'Label name already exists' });
        }
        const request = await db_1.db.customLabelRequest.create({
            data: {
                name,
                userId,
                status: 'PENDING'
...
router.patch('/:id/approve', async (req, res) => {
    try {
        ...
        const { id } = req.params;
        // Get the request
        const request = (await db_1.db.customLabelRequest.findMany({ where: { id } }))[0]; // TODO: Add findUnique to customLabelRequestOperations
        if (!request) {
        ...
        const subLabel = await db_1.db.subLabel.create({
            data: {
                name: request.name,
        ...
        const updatedRequest = await db_1.db.customLabelRequest.update({
            where: { id },
