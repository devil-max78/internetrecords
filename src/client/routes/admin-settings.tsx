...
function AdminSettingsComponent() {
  ...
  const { data: subLabels } = useQuery({
    queryKey: ['adminSubLabels'],
    queryFn: () => api.admin.getAllSubLabels(),
  });
  ...
  const { data: labelRequests } = useQuery({
    queryKey: ['customLabelRequests'],
    queryFn: () => api.customLabels.getAll(),
  });

  // Mutations
  const createSubLabelMutation = useMutation({
    mutationFn: ({ name, userId }: { name: string; userId?: string }) =>
      api.admin.createSubLabel(name, userId),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['adminSubLabels'] });
      setNewSubLabel('');
  ...
  const approveLabelRequestMutation = useMutation({
    mutationFn: (id: string) => api.customLabels.approve(id),
    onSuccess: (data) => {
      queryClient.invalidateQueries({ queryKey: ['customLabelRequests'] });
      queryClient.invalidateQueries({ queryKey: ['adminSubLabels'] }); // Update sub-labels list
      toast.success(`Label "${data.label.name}" approved`);
    },
    onError: (error: any) => {
      toast.error(error.message || 'Failed to approve label');
    },
  });

  const rejectLabelRequestMutation = useMutation({
    mutationFn: ({ id, reason }: { id: string; reason: string }) => api.customLabels.reject(id, reason),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['customLabelRequests'] });
      toast.success('Label request rejected');
    },
    onError: (error: any) => {
      toast.error(error.message || 'Failed to reject label');
    },
  });
  ...
  const handleAddSubLabel = (e: React.FormEvent) => {
    e.preventDefault();
    if (newSubLabel.trim()) {
      createSubLabelMutation.mutate({
        name: newSubLabel.trim(),
  ...
  return (
    <div className="max-w-6xl mx-auto space-y-6">
      ...
      <div className="bg-white rounded-lg shadow-md p-6">
        <h2 className="text-xl font-semibold mb-4">Custom Label Requests</h2>

        {labelRequests?.filter((r: any) => r.status === 'PENDING').length === 0 ? (
          ...
          <div className="overflow-x-auto">
            <table className="min-w-full divide-y divide-gray-200">
              <thead className="bg-gray-50">
                <tr>
                  <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Requested Label</th>
              ...
              <tbody className="bg-white divide-y divide-gray-200">
                {labelRequests?.filter((r: any) => r.status === 'PENDING').map((req: any) => (
                  <tr key={req.id}>
                    <td className="px-6 py-4 whitespace-nowrap font-medium">{req.name}</td>
                    <td className="px-6 py-4 whitespace-nowrap text-gray-600">{req.user?.name || 'Unknown'}</td>
                    <td className="px-6 py-4 whitespace-nowrap text-gray-500">{new Date(req.createdAt).toLocaleDateString()}</td>
                    <td className="px-6 py-4 whitespace-nowrap space-x-2">
                      <button
                        onClick={() => approveLabelRequestMutation.mutate(req.id)}
                        disabled={approveLabelRequestMutation.isPending}
                      ...
                      <button
                        ...
                        disabled={rejectLabelRequestMutation.isPending}
              ...
              {users?.map((u: any) => (
                <tr key={u.id} className="hover:bg-gray-50">
                  ...
                  <td className="px-6 py-4 whitespace-nowrap">
                    <span className={`px-2 py-1 text-xs font-semibold rounded-full ${u.role === 'ADMIN' ? 'bg-purple-100 text-purple-800' :
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div className="bg-white rounded-lg shadow-md p-6">
          ...
          <form onSubmit={handleAddSubLabel} className="mb-4 space-y-3">
          ...
          <div className="space-y-2">
            ...
            <ul className="space-y-1">
              {subLabels?.map((label: any) => {
                const assignedUser = users?.find((u: any) => u.id === label.userId);
                return (
                    ...
                    <div className="flex flex-col">
                      ...
                      {label.userId ? (
                        <span className="text-xs text-purple-600 font-medium">
                          Private {assignedUser ? `(User: ${assignedUser.name})` : `(User ID: ${label.userId})`}
                        </span>
                      ) : (
