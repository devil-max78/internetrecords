...
async function verifyCustomLabelFlow() {
    ...
    try {
        ...
        if (request.status !== 'PENDING')
            throw new Error('Request status should be PENDING');
        ...
        const { data: subLabel, error: subLabelError } = await supabase
            ...
            .single();
        if (subLabelError)
            throw new Error(`Failed to create sub_label: ${subLabelError.message}`);
        ...
        if (artistFetchError)
            throw new Error(`Failed to fetch artist labels: ${artistFetchError.message}`);
        const foundForArtist = artistLabels.find(l => l.name === labelName);
        if (!foundForArtist)
            throw new Error('Artist could not find their own custom label');
        ...
        if (otherFetchError)
            throw new Error(`Failed to fetch other labels: ${otherFetchError.message}`);
        const foundForOther = otherLabels.find(l => l.name === labelName);
        if (foundForOther)
            throw new Error('Other user SHOULD NOT see this label, but did.');
